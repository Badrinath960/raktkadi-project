  ...
  const navItems = {
    ...
    [USER_TYPES.BLOOD_BANK]: [
      ...
      { name: "Staff Management", path: "/blood-bank/staff", icon: "users" },
      {
        ...
        path: "/blood-bank/create-blood-bag",
        icon: "blood-stock",
      },
    ],
    [USER_TYPES.STAFF]: [
    ...
    [USER_TYPES.HOSPITAL]: [
      ...
      {
        name: "Inventory Management",
        path: "/hospital/create-blood-bag",
        icon: "blood-stock",
      },
    ],
    [USER_TYPES.CUSTOMER]: [
  ...
  const DesktopSidebar = () => (
    ...
    >
      <button
        ...
        className={`${
          !isSidebarOpen ? "right-9" : "right-4"
        } absolute top-4  z-10 hover:rotate-90 transition-transform`}
      >
      ...
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-center h-16 bg-red-800 shadow-md">
          <Droplet
            className={`mr-2 ${!isSidebarOpen ? "opacity-0" : ""}`}
            color="white"
            size={42}
          />
          <div>
        ...
        {currentNavItems.length > 0 ? (
          <nav className="flex-1 overflow-y-auto">{renderNavItems()}</nav>
        ) : (
